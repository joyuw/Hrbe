...
def raplaceCodefunc(c,m,k,channel):
   ...
   '''
   if r.get(f'{m.chat.id}:replace:{m.from_user.id}{Dev_FLER}') or r.get(f'{m.chat.id}:replace2:{m.from_user.id}{Dev_FLER}') or r.get(f'{m.chat.id}:replace3:{m.from_user.id}{Dev_FLER}'):
     if text == 'الغاء':
       r.delete(f'{m.chat.id}:replace:{m.from_user.id}{Dev_FLER}')
       r.delete(f'{m.chat.id}:replace2:{m.from_user.id}{Dev_FLER}')
       r.delete(f'{m.chat.id}:replace3:{m.from_user.id}{Dev_FLER}')
   ...
   if text == 'استبدال كلمه' or text == 'استبدال كلمة':
      if not devp_pls(m.from_user.id,m.chat.id):
         ...
         r.set(f'{m.chat.id}:replace:{m.from_user.id}{Dev_FLER}',1,ex=600)
         return m.reply(f'{k} ارسل الكلمة القديمة الآن')
   
   if r.get(f'{m.chat.id}:replace:{m.from_user.id}{Dev_FLER}') and devp_pls(m.from_user.id,m.chat.id):
      r.set(f'{m.chat.id}:replace2:{m.from_user.id}{Dev_FLER}',m.text,ex=600)
      r.delete(f'{m.chat.id}:replace:{m.from_user.id}{Dev_FLER}')
      return m.reply(f'{k} ارسل الكلمة الجديدة هسة')
   
   if r.get(f'{m.chat.id}:replace2:{m.from_user.id}{Dev_FLER}') and devp_pls(m.from_user.id,m.chat.id):
      txt = r.get(f'{m.chat.id}:replace2:{m.from_user.id}{Dev_FLER}')
      r.delete(f'{m.chat.id}:replace2:{m.from_user.id}{Dev_FLER}')
      r.set(f'{m.chat.id}:replace3:{m.from_user.id}{Dev_FLER}',f'{txt}&&new&&{m.text}',ex=600)
      a = os.listdir('Plugins')
      a.sort()
      txt = f'{k} ارسل اسم الملف الي تريد تعدل فيه هسة:'
      count = 1
      txt += '\n\n——— ملفات السورس ———'
      for file in a:
          if file.endswith('.py'):
   ...
   if r.get(f'{m.chat.id}:replace3:{m.from_user.id}{Dev_FLER}') and devp_pls(m.from_user.id,m.chat.id) and m.text in os.listdir('Plugins'):
      mm = m.reply(f'{k} جاريع تعديل الملف')
      get = r.get(f'{m.chat.id}:replace3:{m.from_user.id}{Dev_FLER}')
      old = get.split('&&new&&')[0]
      new = get.split('&&new&&')[1]
      r.delete(f'{m.chat.id}:replace3:{m.from_user.id}{Dev_FLER}')
